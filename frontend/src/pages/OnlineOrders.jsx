...
import {
  Bike, RefreshCw, Search,
  Clock, MapPin, Phone, Inbox, IndianRupee, Sparkles, Printer, AlertTriangle, CheckCircle2,
} from "lucide-react";
...
const SLA_STYLES = {
  ok: { bg: "bg-forest-light", text: "text-forest", border: "border-forest/30", icon: CheckCircle2 },
  warn: { bg: "bg-amber-100", text: "text-amber-800", border: "border-amber-300", icon: Clock },
  overdue: { bg: "bg-red-100", text: "text-red-700", border: "border-red-300", icon: AlertTriangle },
};
...
export default function OnlineOrders() {
  ...
  useEffect(() => {
    refresh();
    const r = setInterval(refresh, 6000);
    const tick = setInterval(() => setNow(Date.now()), 30000);
    return () => { clearInterval(r); clearInterval(tick); };
  }, []);
    ...
    <div className="px-6 lg:px-10 py-8 max-w-[1440px] mx-auto">
      <div className="flex items-end justify-between flex-wrap gap-6 mb-8">
        ...
        <div className="flex flex-wrap gap-2">
          <Button onClick={() => simulate("swiggy")} disabled={loading} data-testid="sim-swiggy"
            style={{ background: "#FC8019" }} className="text-white hover:opacity-90 transition-all">
            <Bike className="w-4 h-4 mr-2" /> + Swiggy order
          </Button>
          <Button onClick={() => simulate("zomato")} disabled={loading} data-testid="sim-zomato"
            style={{ background: "#CB202D" }} className="text-white hover:opacity-90 transition-all">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-8" data-testid="stat-strip">
        ...
        <Card className="p-5 border-border shadow-none border-l-4 border-l-forest">
          ...
          <div className="text-xs text-muted-foreground mt-1">Accepted · 20-min SLA</div>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4" data-testid="orders-grid">
        ...
        {filtered.map(o => {
          ...
          const sla = o.status === "accepted" ? slaStatus(o.accepted_at || o.created_at) : null;
          const slaStyle = sla ? SLA_STYLES[sla.level] : null;
          const SlaIcon = slaStyle?.icon;
          return (
            <Card key={o.id} data-testid={`order-${o.id}`}
              className={`border-border shadow-none overflow-hidden hover:-translate-y-0.5 transition-all duration-200 ${sla?.level === 'overdue' ? 'ring-2 ring-red-300' : ''}`}>
              ...
              <div className="p-5">
                ...
                {sla && slaStyle && (
                  <div className={`mt-3 px-3 py-2 rounded-md flex items-center justify-between border ${slaStyle.bg} ${slaStyle.border}`}
                    data-testid={`sla-${o.id}`}>
                    <span className={`text-xs font-semibold flex items-center gap-1.5 ${slaStyle.text}`}>
                      <SlaIcon className="w-3.5 h-3.5" />
                      {sla.level === 'overdue' ? 'SLA breached' : sla.level === 'warn' ? 'Hurry up' : 'On track'}
                    </span>
                    <span className={`text-xs font-mono font-bold ${slaStyle.text}`}>{sla.label}</span>
                  </div>
                )}
                ...
                {o.status === "accepted" && (
                  <div className="grid grid-cols-2 gap-2 mt-4">
                    <Button onClick={() => printKOT(o)} variant="outline" className="border-border" data-testid={`print-kot-${o.id}`}>
                      <Printer className="w-4 h-4 mr-1.5" /> Print KOT
                    </Button>
                    <Button onClick={() => act(o, "dispatch")} className="bg-foreground hover:bg-foreground/90 text-white" data-testid={`dispatch-${o.id}`}>
                      Dispatch
                    </Button>
                  </div>
                )}
      ...
      <footer className="mt-12 pt-6 border-t border-border text-xs text-muted-foreground flex items-center justify-between">
        <div><span className="font-semibold">Demo mode</span> — orders are simulated. Connect Swiggy/Zomato partner webhooks once onboarded. <span className="font-mono ml-2">Updated {new Date(now).toLocaleTimeString('en-IN')}</span></div>
        <div className="font-mono">FORK&amp;FIRE POS · v0.2</div>
      </footer>
    </div>
  );
}
